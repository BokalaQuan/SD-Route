...
class WebSocketTopology(app_manager.RyuApp):
    ...
    # _EVENTS = [ws_event.EventWebRouteSet]
    ...
    @set_ev_cls(ws_event.EventWebRouteSet)
    def _event_web_reoutr_set(self,ev):
        # links = 'aaaaaaaaaaaaa'
        # print 'event_route_set, rpc broadcall: ', links.to_dict()
        send_link_msg = []
        for link in ev.links:
            # print link
            single_link_msg = {'src_dpid': str(link[0]),
                                'src_port_no': str(link[1]),
                                'dst_dpid': str(link[2]),
                                'dst_port_no': str(link[3])}
            # single_link_msg = {'src_dpid': (link[0]),
            #                     'src_port_no': (link[1]),
            #                     'dst_dpid': (link[2]),
            #                     'dst_port_no': (link[3])}
            send_link_msg.append(single_link_msg)
        # print 'event_route_set, rpc broadcall: ', send_link_msg
        self._rpc_broadcall('event_route_set', send_link_msg)

    @set_ev_cls(ws_event.EventWebRouteSetDij)
    def _event_web_reoutr_set_dij(self,ev):
        # links = 'aaaaaaaaaaaaa'
        # print 'event_route_set_dij, rpc broadcall: ', ev.links
        send_link_msg = []
        for link in ev.links:
            # print link
            single_link_msg = {'src_dpid': str(link[0]),
            ...
            send_link_msg.append(single_link_msg)
        # print 'event_route_set, rpc broadcall: ', send_link_msg
        self._rpc_broadcall('event_route_set_dij', send_link_msg)
        # print'aaaa'
        # self._rpc_broadcall('event_route_set', 'aaaa')
        # print'rpc broadcall ok!'

    @set_ev_cls(ws_event.EventWebLinkBandChange)
    def _event_web_link_change_set(self,link_band):
        # print 'event_link_band_change rpc broadcall: ', link_band
        self._rpc_broadcall('event_link_band_change', link_band)
